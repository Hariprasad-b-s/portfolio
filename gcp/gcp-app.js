...
try {
    ...
    console.log("Firebase & Analytics initialized (GCP Portfolio)");
} catch (error) {
...
async function fetchData() {
    try {
        ...
        renderGCPServices(data.gcpServices);
        ...
        document.body.innerHTML = '<h1 style="text-align:center; padding-top: 20%; color: #4285F4;">Error loading GCP profile data.</h1>';
    }
}
...
function renderAbout(summary, profile) {
    ...
    const jsonView = {
        name: profile.name,
        role: profile.role,
        focus: "GCP Data Engineering",
        coreSkills: ["Apache Beam", "Google Cloud Dataflow", "BigQuery", "Composer (Airflow)"],
        experience: "2+ Years (Comcast)",
...
function renderGCPServices(services) {
    const container = document.getElementById('gcp-services-container');
    services.forEach(svc => {
        const card = document.createElement('div');
        card.className = 'gcp-service-card';
        card.innerHTML = `
...
// Certifications removed from GCP resume
...
window.openResumeModal = function (e) {
    ...
    if (analytics) { logEvent(analytics, 'view_gcp_resume'); }
}
...
async function handleSubmit(event) {
    ...
    try {
        ...
        form.reset();
        if (analytics) { logEvent(analytics, 'gcp_contact_form_success'); }
    } catch (error) {
        console.error("Error adding document: ", error);
        status.innerHTML = "Transmission Failed: " + error.message;
        status.className = "error";
        if (analytics) { logEvent(analytics, 'gcp_contact_form_error', { error: error.message }); }
    }
